...
int decarangingmode(uint8 s1switch)
{
    ...
    if(s1switch & SWS1_SHF_MODE)     //SHF, short frame mode, means 6.81Mbs data rate.
    {
        mode = 1;
    }
...
uint32 inittestapplication(uint8 s1switch)
{
    ...
    port_set_dw1000_slowrate();  //SPI1 is clocked from 72MHz, /32 = 2.28MHz, max SPI before PLLs configured is ~4M

    //this is called here to wake up the device (i.e. if it was in sleep mode before the restart)
    devID = instancereaddeviceid() ;
    if(DWT_DEVICE_ID != devID) //if the read of device ID fails, the DW1000 could be asleep
    {
        port_wakeup_dw1000();// CS pin on DW1000, reset SPI and dw1000

        devID = instancereaddeviceid() ;
        // SPI not working or Unsupported Device ID
        if(DWT_DEVICE_ID != devID)    //0xDECA0130
            return(-1) ;
    ...
    reset_DW1000();  //RSTn pin on DW1000

    result = instance_init() ; //if all down, then return 0;
    if (0 > result) return(-1) ; // Some failure has occurred

    port_set_dw1000_fastrate();  //72/4=18MHz SPI
    devID = instancereaddeviceid() ;
    ...
    if(s1switch & SWS1_ANC_MODE)  //read out the SW3 setting, and distinguish the working mode.
    {
...
void configure_continuous_txspectrum_mode(uint8 s1switch)
{
	...
	instance_starttxtest(0x1000);   // initiate the board.
	//measure the power
...
int dw_main(void)
{
    ...
    peripherals_init();    //arm initial

    spi_peripheral_init(); //set spix(spi1),spiy(spi2, lcd, master)
    ...
    Sleep(1000);
#endif
// rewrite s1switch value, instead of reading the port value. When switch (S1) is 'on' the pin is low
    ...
    //Rewriet TA_SW1_3 value, when switch (S1) is 'on' the pin is low.
    if(port_is_switch_on(TA_SW1_3) == S1_SWITCH_OFF)  //usb mode
    {
        int j = 1000000;
        uint8 command;
    ...
    else //run DecaRanging application
    {
        ...
        if(inittestapplication(s1switch) == (uint32)-1)  //if read device ID failed
        {
        ...
        if((s1switch & SWS1_TXSPECT_MODE) == SWS1_TXSPECT_MODE) //use for decaranging pc app
        {
        ...
        if(instance_mode == TAG)
        {
			...
			writetoLCD( 40, 1, dataseq); //send some data
			sprintf((char*)&dataseq[0], "%llX", instance_get_addr()); //we need to run this function.
			writetoLCD( 16, 1, dataseq); //send some data
        }
        else
        {
    ...
    port_EnableEXT_IRQ();  //after setting, we need to enable IRQ
    // main loop
    while(1)
    {
