...
int instancesendpacket(uint16 length, uint8 txmode, uint32 dtime)
{
    ...
    if (dwt_starttx(txmode))  // delayed start was too late  TX has error.
    {
        result = 1; //late/error
    }
...
int testapprun(instance_data_t *inst, int message)
{
    ...
    switch (inst->testAppState)
    {
        case TA_INIT :
            ...
            switch (inst->mode)
            {
                case TAG:
                {
                    ...
                    inst->mode = TAG_TDOA ;                  //only TAG_TDOA can sending the blinking message.
        ...
        case TA_TXBLINK_WAIT_SEND :
            {
                int flength = (BLINK_FRAME_CRTL_AND_ADDRESS + FRAME_CRC); // 0xC5 + seq num + add + FCR
        ...
        case TA_TXPOLL_WAIT_SEND :
            {
                ...
                dwt_setrxaftertxdelay(inst->txToRxDelayTag_sy); //after TX, set the RX turn on delay time.
                dwt_setrxtimeout((uint16)inst->fwtoTime_sy);    // I do not know why not fwtoTimeB_sy
        ...
        case TA_TX_WAIT_CONF :
           //printf("TA_TX_WAIT_CONF %d m%d states %08x %08x\n", inst->previousState, message, dwt_read32bitreg(0x19), dwt_read32bitreg(0x0f)) ;
                ...
                if(inst->previousState == TA_TXFINAL_WAIT_SEND)  //start a new ranging, the device is tag.
                {
                    inst->testAppState = TA_TXE_WAIT ;
                    inst->nextState = TA_TXPOLL_WAIT_SEND ;
                    break;
                }
                else if (inst->gotTO) //timeout, the re-transmission
                {
            ...
            switch (message)
            {
                case DWT_SIG_RX_BLINK :
                {
                    ...
                    if((inst->mode == LISTENER) || (inst->mode == ANCHOR))
                    {
                        ...
                        instaddtagtolist(inst, &(dw_event->msgu.rxblinkmsg.tagID[0]));  //add tag's address into tag list.
                        ...
                        switch(fcode)
                        {
                            case RTLS_DEMO_MSG_RNG_INIT:
                            {
                                if(inst->mode == TAG_TDOA) //only start ranging with someone if not ranging already
                                {
                                    ...
                                    inst->txToRxDelayTag_sy = US_TO_SY_INT((uint64)resp_dly[RESP_DLY_ANC] - inst->frameLengths_us[POLL]) - RX_START_UP_SY; //RX_START_UP_SY is the default value only in RX mode, in tx, there is no such a value
                                    ...
                                    inst->finalReplyDelay_ms = CEIL_DIV(final_reply_delay_us, 1000);  //convert us to ms
                                    // If we are using long response delays, deactivate sleep.
                                    if (resp_dly[RESP_DLY_ANC] >= LONG_RESP_DLY_LIMIT_US  //25ms
